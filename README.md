...
6. 컬럼이 한국어였기 때문에 인코딩 방식을 UTF-8 로 수정함 ▷ CSV 파일이 이상하게 추출됨 ▷ 
7. 아 그러면 빈 엑셀파일을 만들어 LOOP 커맨드 등을 사용해 데이터테이블을 EXCEL 파일에 넣어보려고 생각해봤지만 그렇게 된다면 너무 너무 길어짐 좀 더 간결한 방법이 없나 생각함 ▷ 
8. 일단 0번 Row가 문자열이기 때문에 오류가 뜰 것이다라는 가정이 사실인지 확인하기 위해 Delete 커맨드를 사용해 첫 번째 줄 삭제해보고 다시 시도해본 결과 동일한 오류 발생 ▷ 
9. 다시 처음으로 돌아와서 생각해보니 다른 하나의 데이터 테이블은 오류가 안뜬 이유에 집중 ▷ 
10. 데이터테이블을 열어본 결과.............. 오류가 발생하지 않은 데이터테이블의 숫자는 다음과 같은 형태 "1303.45", 오류가 발생한 데이터테이블의 숫자는 다음과 같았다.
